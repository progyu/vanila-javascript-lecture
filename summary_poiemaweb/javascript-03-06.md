...
- HTML, CSS 파일은 렌더링 엔진의 HTML 파서와 CSS 파서에 의해 파싱(Parsing)되어 DOM, CSSOM 트리로 변환되고 렌더 트리로 결합된다. 이렇게 생성된 렌더 트리를 기반으로 브라우저는 웹 페이지를 표시한다.
...
- let, const 키워드가 도입되기 이전까지 변수를 선언할 수 있는 유일한 키워드인 **var**은 블록 레벨 스코프(Block-level scope)를 지원하지 않는 점, 호이스팅 문제 등의 단점이 있다.
...
- 컴퓨터는 숫자, 텍스트, 이미지, 동영상 등의 **모든 데이터를 2진수로 처리**한다. **예를 들어** 2진수 0100 0001를 숫자로 해석하면 65이지만 문자로 해석하면 ‘A’이다.
